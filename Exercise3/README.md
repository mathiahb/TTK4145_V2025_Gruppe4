...
Below is another such list of questions and considerations, but aimed instead at what your program does (on an "individual" level), and how it is structured.
 ...
 - The implementation
   ...
   - These modules are part of the same software system. How will they interact?
     - Methods, functions, messages, channels, network, shared variables, file systems, ...?
        ANSWER: our modules interact through channels and shared variables.
 ...
 - Data from the outside world
   - Something needs to be sent and received on the network
     ...
     ANSWER: We will have messages used for sending information, confirming information and heartbeats (signaling that the elevator is connected to the network). 
     - Should the program do different things when receiving different kinds of messages?
     ANSWER: Heartbeats: if received: do nothing. If not received: disconnect node from the network.
     Information: If the message is accepted send confirmation. If confirmation is received do nothing. If a confirmation is not received when expected: troubleshoot: resend. 
   - You also get data from (and to) the elevator hardware
     - Should the different kinds of data feed into the same "top-level" module? Yes for hall orders
     - Or should they go directly to their own specific modules? Yes for cab orders and stop/obstruction.
